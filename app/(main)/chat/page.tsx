...
const ChatPage = () => {
	...
	useEffect(() => {
		...
		return () => {
			...
			queryClient.invalidateQueries({ queryKey: [UNREAD_CHATS_COUNT_BASE_KEY] });
		};
	}, [queryClient]);
...
const ProtectedChatPage: React.FC = () => {
	return (
		<ProtectedRoute>	
			<Suspense fallback={<div className="flex items-center justify-center h-screen"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-indigo-500"></div></div>}>
				<ChatPage />
			</Suspense>
		</ProtectedRoute>
	);
